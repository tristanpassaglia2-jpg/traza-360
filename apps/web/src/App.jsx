...
// ═══════════════════════════════════════════════
  // CITA SEGURA — Timer de seguridad preventivo
  // ═══════════════════════════════════════════════
  const [timerActivo, setTimerActivo] = useState(false);
  const [timerEndTime, setTimerEndTime] = useState(null);
  const [timerDuracion, setTimerDuracion] = useState(0);
  const [timerRestante, setTimerRestante] = useState(0);
  const [showCheckIn, setShowCheckIn] = useState(false);
  const [showTimerSetup, setShowTimerSetup] = useState(false);

  // Restaurar timer si estaba corriendo (recarga / volver a la app)
  useEffect(function() {
    try {
      var saved = localStorage.getItem("vigia_timer");
      if (saved) {
        var d = JSON.parse(saved);
        var remaining = Math.floor((d.endTime - Date.now()) / 1000);
        if (remaining > 0) {
          setTimerEndTime(d.endTime);
          setTimerDuracion(d.duracion);
          setTimerRestante(remaining);
          setTimerActivo(true);
        } else {
          localStorage.removeItem("vigia_timer");
          if (remaining > -1800) ejecutarAlertaTimer();
        }
      }
    } catch(e) {}
  }, []);

  // Countdown — background-safe (recalcula al volver a la app)
  useEffect(function() {
    if (!timerActivo || !timerEndTime) return;
    var fired = false;
    function tick() {
      var r = Math.max(0, Math.floor((timerEndTime - Date.now()) / 1000));
      setTimerRestante(r);
      if (r <= 120 && r > 0) setShowCheckIn(true);
      if (r <= 0 && !fired) {
        fired = true;
        setTimerActivo(false);
        try { localStorage.removeItem("vigia_timer"); } catch(e) {}
        ejecutarAlertaTimer();
      }
    }
    tick();
  ...
  async function ejecutarAlertaTimer() {
    ...
    try {
      await supabase.from("alertas").insert({
        id: alertaId, usuario_id: authUser?.id || null,
        tipo: "timer_cita_segura", modulo: "cita_segura",
        mensaje: "ALERTA - Timer de seguridad expirado. No respondió. Verificar situación urgente.",
    ...
    for (var i = 0; i < contactosParaEnviar.length; i++) {
      try {
        ...
        var response = await fetch("https://vzqxxkxdxcmaucubufpz.supabase.co/functions/v1/send-whatsapp", {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            to: numLimpio, template: "alerta_emergencia",
            params: [nombre.substring(0,60), "Timer de seguridad expirado - No respondió - Ver: traza360.app/alerta/" + alertaId, hora, "Cita Segura"],
            alerta_id: alertaId
          })
        });
        var data = await response.json();
  ...
  function iniciarTimer(segundos) {
    var endTime = Date.now() + segundos * 1000;
    setTimerEndTime(endTime);
    setTimerDuracion(segundos);
    setTimerRestante(segundos);
    setTimerActivo(true);
    setShowTimerSetup(false);
    setShowCheckIn(false);
    try { localStorage.setItem("vigia_timer", JSON.stringify({ endTime: endTime, duracion: segundos })); } catch(e) {}
  }
  ...
  function extenderTimer() {
    var newEnd = (timerEndTime || Date.now()) + 1800000;
    var newDuracion = timerDuracion + 1800;
    setTimerEndTime(newEnd);
    setTimerDuracion(newDuracion);
    setTimerRestante(Math.floor((newEnd - Date.now()) / 1000));
    setShowCheckIn(false);
    try { localStorage.setItem("vigia_timer", JSON.stringify({ endTime: newEnd, duracion: newDuracion })); } catch(e) {}
  }
